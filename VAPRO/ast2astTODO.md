...
- [x] decided: do **not** reuse R's internal `R_zeroin2`. Per CRAN's R NEWS (R 3.0.0, "CHANGES IN R 3.0.0" > C-level facilities): "Several non-API entry points have been transferred to packages (e.g., `R_zeroin2`) [...]" — it has been a non-API entry point since 2013, unlike `solve`/`chol`'s LAPACK/BLAS calls which go through R's documented, stable public C API (`R_ext/Lapack.h`). Calling non-API entry points is explicitly against CRAN policy (packages doing so risk removal) and the symbol/signature isn't guaranteed stable across R versions. Instead: implement Brent's method (bisection + inverse quadratic interpolation, safeguarded) directly as a small header in `inst/include/etr_bits/Core/` — same algorithm, but owned by ast2ast rather than borrowed via an unstable symbol. Small, standard, well-understood to implement (~100 lines), so this isn't a meaningful extra cost.
