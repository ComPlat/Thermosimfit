...
library(ggplot2)
library(patchwork)
setwd("/home/konrad/Documents/GitHub/RProjects/Thermosimfit/Tests/Reproducability/Test1")
files <- list.files(pattern = "*.txt")

l <- lapply(files, function(x) {
  d <- read.table(x, header = FALSE)
  d <- apply(d, 2, as.character)
  d <- rbind(c("Var", "Signal"), d)
  return(d)
})
l <- Reduce(rbind, l)
write.csv(l, file = "Combined", row.names = FALSE, quote = FALSE)
...
res <- opti("ida", c(1, 0, 0, 0), c(10^10, 10 * 10^-4, 100, 100), files[[1]], c(H0, d0, kHD),
  npop = 40, ngen = 100, Topology = "random"
) # 400
res
res[[2]]
