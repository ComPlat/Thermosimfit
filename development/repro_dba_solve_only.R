...
cat("\nR solve_h_dba: which rows produce NA d?\n")
print(which(is.na(r_sol$d)))
cat("d:\n"); print(r_sol$d)
cat("hd:\n"); print(r_sol$hd)

# ---- minimal standalone compiled solve-only function, returning d/hd/root
# per row instead of an aggregated loss, so we can see exactly where it
# diverges from the R version above. -------------------------------------
solve_only <- function(Kd, host, d0) {
  ...
  for (i in 1L:n) {
    ...
    out$root[i] <- ur$root
    if (is.na(ur$root) || is.nan(ur$root) || ur$root <= 0) {
      out$d[i] <- -1
      out$hd[i] <- -1
      out$valid[i] <- 0
    } else {
      d_sol <- d0 / (1 + Kd * ur$root)
      if (is.na(d_sol) || is.nan(d_sol) || d_sol < 0) {
        out$d[i] <- -1
        out$hd[i] <- -1
        out$valid[i] <- 0
      } else {
...
cat("\na2a solve: which rows invalid?\n")
print(which(a2a_sol$valid == 0))
cat("root:\n"); print(a2a_sol$root)
cat("d:\n"); print(a2a_sol$d)
cat("hd:\n"); print(a2a_sol$hd)
