...
create_single_param_plots <- function(case, params, env) {
  lossFct <- tryCatch(
    expr = {
...
plot_dba <- function(path) {
  ...
  env$signal <- df[, 2]
  env$d0 <- ap[1]
  create_single_param_plots("dba_dye_const", parameter, env)
}
...
plot_ida <- function(path) {
  ...
  env$d0 <- ap[2]
  env$kd <- ap[3]
  create_single_param_plots("ida", parameter, env)
}
...
plot_gda <- function(path) {
  ...
  env$ga0 <- ap[2]
  env$kd <- ap[3]
  create_single_param_plots("gda", parameter, env)
}
