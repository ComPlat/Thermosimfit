source("./app/HG_App.R")
source("./app/IDA_App.R")
source("./app/GDA_App.R")
...
server <- function(input, output, session) {
  ...
  output$df <- renderDT({
      req(input$upload)
      df <- tsf:::importData(input$upload$datapath) 
      if (is.data.frame(df)) {
        if(ncol(df) != 2) {
  ...
  HG_com <- tsf:::Communicator$new()
  HG_com_sense <- tsf:::Communicator$new()
  GDA_com <- tsf:::Communicator$new()
  GDA_com_sense <- tsf:::Communicator$new()
  IDA_com <- tsf:::Communicator$new()
  IDA_com_sense <- tsf:::Communicator$new()
