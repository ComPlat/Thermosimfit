...
pso <- function(env, lb, ub, loss, ngen, npop, error_threshold, global = FALSE, 
  ...
  if(length(lb) != length(ub)) return(ErrorClass$new("length of lb and ub differ"))
  if(!is.numeric(lb)) return(ErrorClass$new("lb has to be of type numeric"))
  if(!is.numeric(ub)) return(ErrorClass$new("ub has to be of type numeric"))
  if(!is.function(loss)) return(ErrorClass$new("loss has to be of type function"))
  if(!is.numeric(ngen)) return(ErrorClass$new("ngen has to be of type numeric"))
  if(!is.numeric(npop)) return(ErrorClass$new("npop has to be of type numeric"))
  if(length(ngen) != 1) return(ErrorClass$new("ngen has to be a scalar"))
  if(length(npop) != 1) return(ErrorClass$new("npop has to be a scalar"))
  if(length(lb) <= 0) return(ErrorClass$new("lb and ub need at least one element"))
  if(ngen <= 10) return(ErrorClass$new("ngen has to be at least 10"))  
  if(npop <= 5) return(ErrorClass$new("npop has to be at least 5"))
  if(!is.numeric(error_threshold)) return(ErrorClass$new("error_threshold has to be of type numeric"))
  if(length(error_threshold) != 1) return(ErrorClass$new("error_threshold has to be a scalar"))
  if(!is.logical(global)) return(ErrorClass$new("global has to be of type logical"))
  if(length(global) != 1) return(ErrorClass$new("global has to be a scalar"))
  if(!is.logical(saveSwarm)) return(ErrorClass$new("saveSwarm has to be of type logical"))
  if(length(saveSwarm) != 1) return(ErrorClass$new("saveSwarm has to be a scalar"))
  if(any(lb > ub)) {
    w <- which(lb > ub)
    w <- paste(w, collapse = ", ")
  ...
  while (iter < ngen) {
    ...
    if (iter %% 1 == 0 & is.logical(runAsShiny)) {
       ...
       if(iter %% 5 == 0) {
          ...
          gbv <- format(round(gbv, digits = 6), nsmall = 6)
          gbv <- Map(function(a, b) {
            paste(a, " = ", b)
          }, names(gbv), gbv) |> unlist() |> paste(collapse = ", ")
          runAsShiny$running( format(round( (100.0/ngen) * iter, digits = 6), nsmall = 6) )
          runAsShiny$setData(
            paste(
              paste0( format(round( (100.0/ngen) * iter, digits = 6), nsmall = 6), "% completed;"),
              gbv,
              "; Error: ", format(round(global_best_error, digits = 6), nsmall = 6),
              collapse = "\n"
            )
          )
       }
