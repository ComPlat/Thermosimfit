...
opti <- function(case, lowerBounds, upperBounds, path, additionalParameters,
  ...
  if(length(lowerBounds) == 0) return(ErrorClass$new("lowerBounds vector seems to be empty"))
  if(length(lowerBounds) > 4) return(ErrorClass$new("lowerBounds vector has more than 4 entries"))
  ...
  if(!is.numeric(npop) && !is.integer(npop)) return(ErrorClass$new("npop has to be of type numeric or integer"))
  if(npop <= 5 || npop > 400) return(ErrorClass$new("npop has to be between 5 and 400"))
  if(!is.numeric(ngen) && !is.integer(ngen)) return(ErrorClass$new("ngen has to be of type numeric or integer"))
  if(ngen <= 5 || npop > 10^7) return(ErrorClass$new("ngen has to be between 5 and 10^7"))
  if(!is.character(Topology)) return(ErrorClass$new("Topology has to be of type character"))
  if(!(Topology %in% c("star", "random"))) return(ErrorClass$new("Topology is neither star or random"))
  Topo <- FALSE
  ...
  if(case == "hg") {
    ...
    df$d <- res[[1]][, 2]
    df$hd <- res[[1]][, 3]
    params <- data.frame(kguest = res[[2]][1], I0 = res[[2]][2],
                         IHD = res[[2]][3], ID = res[[2]][4])
